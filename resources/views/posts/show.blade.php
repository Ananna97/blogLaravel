...
@section('content')
	...
	<div class="row">
		<div class="col-md-8">
			...
			@if(!empty($post->image))
				<img src="{{asset('images/creatures/' . $post->image)}}" alt="This is a photo." class="imgpost"  />
			@endif
			<h1>{{ $post->title }}</h1>
		...
		<div class="col-md-4">
			<div class="well">
				<dl class="dl-horizontal">
					<label>Url:</label>
					<p><a style="color:white" href="{{ route('blog.single', $post->slug) }}">{{ route('blog.single', $post->slug) }}</a></p>
				</dl>

				<dl class="dl-horizontal">
					<label>Category:</label>
							<a class="tagName" href="{{ route('categories.show', $post->category->id) }}">{{ $post->category->name }}</a>
				</dl>
